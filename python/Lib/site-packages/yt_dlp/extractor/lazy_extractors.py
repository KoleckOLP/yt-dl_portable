...
class ARDBetaMediathekIE(ARDMediathekBaseIE):
    _module = 'yt_dlp.extractor.ard'
    _VALID_URL = 'https://(?:(?:beta|www)\\.)?ardmediathek\\.de/(?P<client>[^/]+)/(?P<mode>player|live|video|sendung|sammlung)/(?P<display_id>(?:[^/]+/)*)(?P<video_id>[a-zA-Z0-9]+)'
...
class BiliBiliSearchIE(LazyLoadSearchExtractor):
    ...
    def _make_valid_url(cls):
        return 'bilisearch(?P<prefix>|[1-9][0-9]*|all):(?P<query>[\\s\\S]+)'
...
class DiscoveryPlusIndiaShowIE(LazyLoadExtractor):
    _module = 'yt_dlp.extractor.discoveryplusindia'
...
class DiscoveryNetworksDeIE(DPlayIE):
    _module = 'yt_dlp.extractor.discoverynetworks'
...
class GronkhIE(LazyLoadExtractor):
    ...
    _VALID_URL = '(?:https?://)(?:www\\.)?gronkh\\.tv/stream/(?P<id>\\d+)'
...
class N1InfoIIE(LazyLoadExtractor):
    ...
    _VALID_URL = 'https?://(?:(?:ba|rs|hr)\\.)?n1info\\.(?:com|si)/(?:[^/]+/){1,2}(?P<id>[^/]+)'
...
class NebulaIE(LazyLoadExtractor):
...
class NicovideoSearchURLIE(LazyLoadExtractor):
    _module = 'yt_dlp.extractor.niconico'
    _VALID_URL = 'https?://(?:www\\.)?nicovideo\\.jp/search/(?P<id>[^?#&]+)?'


class NicovideoSearchIE(LazyLoadSearchExtractor, NicovideoSearchURLIE):
    _module = 'yt_dlp.extractor.niconico'
    _VALID_URL = 'https?://(?:www\\.)?nicovideo\\.jp/search/(?P<id>[^?#&]+)?'
    ...
    @classmethod
    def _make_valid_url(cls):
        return 'nicosearch(?P<prefix>|[1-9][0-9]*|all):(?P<query>[\\s\\S]+)'
...
class NicovideoSearchDateIE(NicovideoSearchIE):
    ...
    @classmethod
    def _make_valid_url(cls):
        return 'nicosearchdate(?P<prefix>|[1-9][0-9]*|all):(?P<query>[\\s\\S]+)'
...
class OlympicsReplayIE(LazyLoadExtractor):
    ...
    _VALID_URL = '(?:https?://)(?:www\\.)?olympics\\.com/tokyo-2020/(?:[a-z]{2}/)?replay/(?P<id>[^/#&?]+)'
...
class PatreonUserIE(LazyLoadExtractor):
    _module = 'yt_dlp.extractor.patreon'
    _VALID_URL = 'https?://(?:www\\.)?patreon\\.com/(?P<id>[-_\\w\\d]+)/?(?:posts/?)?'
...
class PolskieRadioIE(LazyLoadExtractor):
    ...
    _VALID_URL = 'https?://(?:www\\.)?polskieradio\\.pl/\\d+/\\d+/Artykul/(?P<id>[0-9]+)'
...
class RCTIPlusSeriesIE(RCTIPlusBaseIE):
    _module = 'yt_dlp.extractor.rcti'
    _VALID_URL = 'https://www\\.rctiplus\\.com/programs/(?P<id>\\d+)/(?P<display_id>[^/?#&]+)'
...
class SenateISVPIE(LazyLoadExtractor):
    _module = 'yt_dlp.extractor.senateisvp'
...
class TrovoChannelBaseIE(LazyLoadExtractor):
    _module = 'yt_dlp.extractor.trovo'
...
class TVPEmbedIE(LazyLoadExtractor):
    ...
    _VALID_URL = '(?:tvp:|https?://[^/]+\\.tvp\\.(?:pl|info)/sess/tvplayer\\.php\\?.*?object_id=)(?P<id>\\d+)'


class TVPIE(LazyLoadExtractor):
    _module = 'yt_dlp.extractor.tvp'
    _VALID_URL = 'https?://[^/]+\\.tvp\\.(?:pl|info)/(?:video/(?:[^,\\s]*,)*|(?:(?!\\d+/)[^/]+/)*)(?P<id>\\d+)'
...
class ZenYandexIE(LazyLoadExtractor):
    _module = 'yt_dlp.extractor.yandexvideo'
    _VALID_URL = 'https?://zen\\.yandex\\.ru/media/(?:id/[^/]+/|[^/]+/)(?:[a-z0-9-]+)-(?P<id>[a-z0-9-]+)'


class ZenYandexChannelIE(LazyLoadExtractor):
    _module = 'yt_dlp.extractor.yandexvideo'
    _VALID_URL = 'https?://zen\\.yandex\\.ru/(?!media)(?:id/)?(?P<id>[a-z0-9-_]+)'
...
class YoutubeSearchIE(LazyLoadSearchExtractor, YoutubeTabIE):
    _module = 'yt_dlp.extractor.youtube'
    _VALID_URL = '(?x)\n                    https?://\n                        (?:\\w+\\.)?\n                        (?:\n                            youtube(?:kids)?\\.com|\n                            invidio\\.us\n                        )/\n                        (?:\n                            (?P<channel_type>channel|c|user|browse)/|\n                            (?P<not_channel>\n                                feed/|hashtag/|\n                                (?:playlist|watch)\\?.*?\\blist=\n                            )|\n                            (?!(?:channel|c|user|playlist|watch|w|v|embed|e|watch_popup|clip|shorts|movies|results|shared|hashtag|trending|feed|feeds|browse|oembed|get_video_info|iframe_api|s/player|storefront|oops|index|account|reporthistory|t/terms|about|upload|signin|logout)\\b)  # Direct URLs\n                        )\n                        (?P<id>[^/?\\#&]+)\n                    '
...
class YoutubeSearchDateIE(YoutubeSearchIE):
    ...
    @classmethod
    def _make_valid_url(cls):
        return 'ytsearchdate(?P<prefix>|[1-9][0-9]*|all):(?P<query>[\\s\\S]+)'
